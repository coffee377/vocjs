...
export default class TypeFormatter {
  public static format(type: Type, options?: TypeFormatterOptions): string {
    const mergedOptions: TypeFormatterOptions = {
      ...defaultOptions,
      ...options,
    };
    ...
    if (type.type === 'conditional') {
      ...
      return `${TypeFormatter.format(conType.checkType)} extends ${TypeFormatter.format(
        conType.extendsType,
      )} ? ${TypeFormatter.format(conType.trueType)} : ${TypeFormatter.format(conType.falseType)}`;
    }
    ...
    if (type.type === 'union') {
      return (type as UnionType).types
        .filter(t => {
          ...
          return (
            !mergedOptions.isOptionalType || !(intrinsicType.type === 'intrinsic' && intrinsicType.name === 'undefined')
          );
        })
    ...
    throw new Error(`Unrecognised type: ${type.type}`);
  }
}
